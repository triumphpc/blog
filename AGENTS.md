...
- **Stack**: Hugo v0.147+, PaperMod theme, hand-drawn SVG diagrams, KaTeX math
...
6. **Схемы — только hand-drawn SVG, Mermaid больше не используем.** Любая схема, диаграмма, архитектура, последовательность — inline `<svg>` прямо в markdown (goldmark unsafe включён), в отдельном стилизованном `<div>`-контейнере. Никаких текстовых псевдосхем вроде `A → B → C` и никаких новых `{{</* mermaid */>}}` shortcode — старые посты, где Mermaid уже стоит, не переделываем (это отдельная фиксированная Tokyo Night тема в `layouts/partials/extend_footer.html`, её не трогаем), но во всех новых статьях — только SVG. Палитра SVG — фиксированная и не зависит от переключателя тема сайта (по аналогии с тем, как у Mermaid зашита Tokyo Night): тёмный фон `#0d0e12`/`#101114`, граница `#262a31`/`#26282e`, текст `#e5e7eb`, приглушённый текст `#8b90a0`, акцент `#4ade80` (зелёный терминала), плюс `#60a5fa`/`#fbbf24`/`#f87171` для дополнительных категорий на схеме; шрифт — `var(--font-mono, 'JetBrains Mono'), monospace`. В RU-версии статьи весь текст внутри SVG (подписи, `aria-label`) переводится на русский — не переиспользуем английский SVG как есть.
